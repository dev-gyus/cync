...
function exec(command, cwd) {
    try {
        ...
        if (stderr.includes('not a git repository')) {
            throw new GitError(`Not a git repository: ${cwd ?? process.cwd()}`, 'NOT_A_REPO', error);
        }
        if (stderr.includes('Could not resolve host') ||
            stderr.includes('unable to access') ||
            stderr.includes('fatal: repository') ||
            stderr.includes('Connection refused')) {
            throw new GitError(`Remote unreachable: ${stderr}`, 'REMOTE_UNREACHABLE', error);
        }
        if (stderr.includes('Authentication failed') || stderr.includes('Permission denied')) {
            throw new GitError(`Authentication failed: ${stderr}`, 'AUTH_FAILED', error);
        }
        if (stderr.includes('CONFLICT') || stderr.includes('Merge conflict')) {
            throw new GitError(`Merge conflict detected: ${stderr}`, 'MERGE_CONFLICT', error);
        }
        throw new GitError(`Git command failed: ${command}\n${stderr}`, 'GIT_COMMAND_FAILED', error);
    }
}
/**
...
function requireGit() {
    if (!isGitInstalled()) {
        throw new GitError('Git is not installed or not found on PATH. Please install git first.', 'GIT_NOT_FOUND');
    }
}
/**
 * Clone a remote repository into a local directory.
 */
export function gitClone(url, dir) {
    requireGit();
 ...
 */
export function gitAddRemote(dir, name, url) {
    requireGit();
 ...
 */
export function gitLog(dir, count = 10) {
    requireGit();
