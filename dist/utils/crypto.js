...
export function getEncryptionKey() {
    ...
    if (!key) {
        throw new Error('CC_SYNC_KEY environment variable is not set. ' +
...
export function encrypt(data, key) {
    ...
    if (keyBuffer.length !== KEY_LENGTH) {
        throw new Error(`Invalid key length: expected ${KEY_LENGTH} bytes (${KEY_LENGTH * 2} hex chars), got ${keyBuffer.length} bytes.`);
    }
...
export function decrypt(data, key) {
    ...
    if (data.length < minLength) {
        throw new Error(`Invalid encrypted data: expected at least ${minLength} bytes, got ${data.length}.`);
    }
