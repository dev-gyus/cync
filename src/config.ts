 ...
 */
export function validateConfig(config: unknown): config is SyncConfig {
  if (typeof config !== 'object' || config === null) return false;

  const c = config as Record<string, unknown>;

  if (typeof c.remote !== 'string') return false;
  if (typeof c.branch !== 'string') return false;
  if (typeof c.machine_id !== 'string') return false;

  // Validate modules
  if (typeof c.modules !== 'object' || c.modules === null) return false;
  const modules = c.modules as Record<string, unknown>;
  const moduleKeys = ['core', 'skills', 'commands', 'memory', 'plugins', 'plans', 'full'];
  for (const key of moduleKeys) {
    if (typeof modules[key] !== 'boolean') return false;
  }

  // Validate sensitive
  if (typeof c.sensitive !== 'object' || c.sensitive === null) return false;
  const sensitive = c.sensitive as Record<string, unknown>;
  if (typeof sensitive.encrypt !== 'boolean') return false;
  if (!Array.isArray(sensitive.exclude)) return false;
 ...
 */
export async function loadConfig(claudeDir: string): Promise<SyncConfig> {
  ...
  const rawSensitive = (raw.sensitive ?? {}) as Record<string, unknown>;
 ...
 */
export function getSyncRepoDir(claudeDir: string): string {
